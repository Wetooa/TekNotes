...
class ChatRoom(LoginRequiredMixin, View):
    ...
    def Create_or_Enter_room(self, sender, receiver):
        ...
        rooms_with_sender = ChatUsers.objects.filter(users=sender).values_list(
            "room", flat=True
        )
        print("Rooms with sender: ", rooms_with_sender)
        rooms_with_receiver = ChatUsers.objects.filter(users=receiver).values_list(
            "room", flat=True
        )
...
class MessageView(View):
    def get(self, request, room_name):
        ...
        Freceiver = (
            ChatUsers.objects.filter(room=get_room)
            .exclude(users=sender)
